...
def blm_init_params(X):
    ...
    temp = np.array([np.min(Xnorm[Xnorm > 0]) for _ in range(D)])
    min_prop_vec = np.append(temp, np.sum(temp[:-1]))
    if (100*min_prop_vec[-2]) > min_prop_vec[-1]:
        min_prop_vec[-1] *= 100
